...
def check_and_correct_user_agents_format():
    ...
    try:
        ...
        if not isinstance(user_agents, list) or any(not isinstance(agent, str) for agent in user_agents):
            log_message(f"Incorrect format in {user_agents_file}. Correcting format...", Fore.RED)
            # Correct the format by resetting to an empty list
            with open(user_agents_file, 'w') as file:
                json.dump([], file)
                log_message("Format corrected. The file has been reset to an empty list.", Fore.GREEN)
    
    except json.JSONDecodeError:
        log_message(f"Invalid JSON format in {user_agents_file}. Correcting format...", Fore.RED)
...
def perform_daily_spin(access_token, proxies=None, user_agent=None):
    log_message("Spin Game completing...", Fore.LIGHTWHITE_EX)
    ...
    if response.status_code == 201:
        ...
        log_message(f"Claim Successful", Fore.GREEN)
...
def process_account(query_id, proxies_list, auto_task, auto_full_task, auto_play_game, durov_enabled, durov_choices, account_proxies, total_balance, user_agents, account_index, proxy_usage):
    ...
    # Select User-Agent: Use default for the first account, then from file
